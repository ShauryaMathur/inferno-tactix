...
model_path = os.environ.get("MODEL_DIR", str(REPO_ROOT / "data" / "models" / "simulator"))

MODEL_FILE = os.path.join(model_path, "ppo_firefighter.zip")

# Create output directory for analytics
ANALYTICS_DIR = os.environ.get("ANALYTICS_DIR", str(REPO_ROOT / "data" / "analytics" / "simulator"))
...
try:
    ...
    class LSTMFeatureExtractor(BaseFeaturesExtractor):
        def __init__(self, observation_space, features_dim=128):
            ...
            self.heli_mlp = nn.Sequential(nn.Linear(2, 16), nn.ReLU())
        ...
        def forward(self, observations):
            ...
            heli_coord = observations["helicopter_coord"].float() / torch.tensor(
                [240.0, 160.0]
            )  # Normalize
    ...
    def step(self, action):
        ...
        if self.step_count % 5 == 0 and hasattr(self, "previous_fire_states"):
            new_burning = np.logical_and(
                fire_states == FireState.Burning, self.previous_fire_states != FireState.Burning
            )
            new_burning_coords = list(zip(*np.where(new_burning), strict=False))
    ...
    try:
        ...
        for episode in range(NUM_EPISODES):
            ...
            if episode < NUM_EPISODES - 1:
                print("Pausing between episodes... (3 seconds)")
                time.sleep(3)
        ...
        if report_files:
                    ...
                    combined_data["episodes"].append(
                        {
                            ...
                            "helitack_operations": episode_data["helitack_operations"][
                                "total_helitacks"
                            ],
                            "helitack_coordinates": episode_data["helitack_operations"][
                                "helitack_coordinates"
                            ],
            ...
            combined_data["summary"] = {
                "avg_burnt_area": sum(
                    ep["summary"]["total_burnt_area"] for ep in combined_data["episodes"]
                )
                / len(combined_data["episodes"]),
                "avg_helitack_operations": sum(
                    ep["helitack_operations"] for ep in combined_data["episodes"]
                )
                / len(combined_data["episodes"]),
