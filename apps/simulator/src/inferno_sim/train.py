...
save_path = os.environ.get("MODEL_DIR", str(REPO_ROOT / "data" / "models" / "simulator"))
...
def apply_aggressive_buffer_fix():
    ...
    try:
        ...
        modified_content = (
            content[:assert_line]
            + "# BUFFER_PATCHED\n        # assert self.full"
            + content[assert_line + len("assert self.full") :]
        )
...
def use_lowest_level_patch():
    ...
    try:
        ...
        for cls in [RolloutBuffer, DictRolloutBuffer]:
            ...
            def new_init(self, *args, _original_init=original_init, **kwargs):
        ...
        print(
            "⚠️ All patching attempts failed. Consider manually editing the stable_baselines3 code."
        )
...
def apply_all_fixes():
    ...
    print(
        "⚠️ If you still experience issues, try re-installing Stable Baselines 3 or manually editing the code."
    )
...
def fix_ppo_buffer_issue(model, verbose=True):
    ...
    if not isinstance(model, PPO):
        if verbose:
            print("⚠️ Model is not a PPO instance. Buffer fix only applies to PPO.")
        return model
    ...
    def fixed_learn(*args, **kwargs):
        ...
        """
        if verbose:
            print("🔄 Using fixed PPO training method that addresses buffer issues")
    ...
    if verbose:
        print("✅ PPO buffer fixes applied successfully")
    return model
...
def obs_as_tensor(obs, device):
    ...
    if isinstance(obs, dict):
        return {key: torch.as_tensor(obs[key], device=device) for key in obs.keys()}
...
def get_optimal_device():
    ...
    if torch.cuda.is_available():
        ...
        print(
            f"🚀 Using CUDA device: {gpu_name} ({gpu_count} device{'s' if gpu_count > 1 else ''} available)"
        )
...
from .FeatureExtractor import FireEnvLSTMPolicy  # noqa: E402

# Directory for logs
logdir = os.environ.get(
    "SIMULATOR_RUNS_DIR", str(REPO_ROOT / "data" / "runs" / "simulator" / "ppo_firefighter")
)
    ...
    def _on_step(self) -> bool:
        ...
        if dones is not None and dones[0]:
            ...
            if self.episode_counter % self.pause_every == 0:
                if self.verbose:
                    print(
                        f"⏸️  Pausing training for {self.pause_duration}s after {self.episode_counter} episodes"
                    )
...
class RewardLoggingCallback(BaseCallback):
    ...
    def _on_step(self):
        ...
        if done:
            ...
            # Log episode length
            self.logger.record(
                "episode/length", self.model.num_timesteps - sum(self.locals["dones"])
            )
...
class MetricsTracker:
    ...
    def save_metrics(self, filename="fire_metrics.json"):
        ...
        helitacks = [[int(step), int(x), int(y)] for step, x, y in self.helitack_actions]
    ...
    def calculate_episode_statistics(self):
        ...
        max_burning = (
            max([b[1] for b in self.burning_cells_over_time]) if self.burning_cells_over_time else 0
        )
...
def websocket_server_thread():
    ...
    async def run_server():
        ...
        server = await websockets.serve(handler, host, port, max_size=None, compression="deflate")
...
class FireEnvSync(gym.Env):
        ...
        self.observation_space = spaces.Dict(
            {
                "helicopter_coord": spaces.Box(
                    low=np.array([0, 0]), high=np.array([239, 159]), dtype=np.int32
                ),
                "cells": spaces.Box(
                    low=0, high=8, shape=(4, 160, 240), dtype=np.int32
                ),  # 4 stacked frames
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.state = self._default_state()
        self.state = {**self.state, **response}
    ...
    def step(self, action):
        ...
        # Create step message
        step_message = json.dumps({"action": str(action), "helicopter_coord": [heli_x, heli_y]})
    ...
    def calculate_reward(self, prev_burnt, curr_burnt, curr_burning, extinguished_by_helitack):
        ...
        if np.any(burning_mask):
            ...
            distances = np.sqrt(
                (burning_coords[:, 0] - heli_y) ** 2 + (burning_coords[:, 1] - heli_x) ** 2
            )
        ...
        print(
            f"Reward: {reward:.2f} | Burning: {curr_burning} | New burnt: {newly_burnt} | Reduction: {burning_reduction}"
        )
        return reward
    ...
    try:
        ...
        if device == "mps":
            ...
            if not torch.backends.mps.is_available():
                ...
                print(
                    f"MPS-optimized batch configuration: batch_size={model.batch_size}, n_steps={model.n_steps}"
                )
        ...
        # Combine callbacks
        callbacks = CallbackList([reward_callback, lr_callback, memory_callback])
        os.makedirs(logdir, exist_ok=True)
