...
class FireEnvLSTMCNN(BaseFeaturesExtractor):
    ...
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 512):
        ...
        self.lstm = nn.LSTM(
            input_size=flattened_dim, hidden_size=256, num_layers=2, batch_first=True, dropout=0.2
        )

        # Helicopter position encoding with relative positioning
        self.coord_net = nn.Sequential(nn.Linear(4, 32), nn.ReLU(), nn.Linear(32, 64), nn.ReLU())

        # On-fire flag processing
        self.fire_net = nn.Sequential(nn.Linear(2, 16), nn.ReLU())
        ...
        self.fc = nn.Sequential(
            nn.Linear(combined_dim, features_dim), nn.ReLU(), nn.Linear(features_dim, features_dim)
        )
    ...
    def forward(self, observations):
        ...
        rel_coords = th.stack(
            [coords[:, 0], coords[:, 1], coords[:, 0] - center_x, coords[:, 1] - center_y], dim=1
        )
