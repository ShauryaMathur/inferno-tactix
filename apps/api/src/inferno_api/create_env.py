...
SERVICE_ACCOUNT_FILE = Path(
    os.environ.get("EE_SERVICE_ACCOUNT_FILE", APP_ROOT / "config" / "ee-service-account.json")
)
GENERATED_TERRAIN_DIR = Path(
    os.environ.get("GENERATED_TERRAIN_DIR", REPO_ROOT / "data" / "generated" / "terrain")
)
...
def get_landcover_data(date_str, region, scale=500, output_dir="."):
    ...
    latest = ee.ImageCollection("MODIS/061/MCD12Q1").sort("system:time_start", False).first()
    img = ee.Image(ee.Algorithms.If(img, img, latest))

    # Select the LC_Type1 band and clip
    landcover = img.select("LC_Type1").clip(ee.Geometry.Polygon(region))

    return download_image(landcover, region, scale, output_dir, f"landcover_{date_str}")
...
def convert_landcover_to_image(
    tif_path, output_width=1200, output_height=800, output_dir="."
) -> str:
    os.makedirs(output_dir, exist_ok=True)
...
def generate_data_and_heightmap(coords: str, date) -> Tuple[str, str, str]:
    ...
    output_dir = str(GENERATED_TERRAIN_DIR)
