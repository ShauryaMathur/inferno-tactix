...
MODELS_DIR = Path(os.environ.get("API_MODELS_DIR", APP_ROOT / "models"))
API_HOST = os.environ.get("API_HOST", "0.0.0.0")
API_PORT = int(os.environ.get("API_PORT", "6969"))
SPAWN_SIMULATOR = os.environ.get("API_SPAWN_SIMULATOR", "false").lower() == "true"
SIMULATOR_MODULE = os.environ.get("SIMULATOR_MODULE", "inferno_sim.simulation")
...
def build_input_tensor(df: pd.DataFrame, date: datetime):
    ...
    return torch.tensor(full_scaled[np.newaxis, ...], dtype=torch.float32)
...
def predict_wildfire():
    ...
    return jsonify(
        {"lat": lat, "lon": lon, "date": dt_obj.strftime("%Y-%m-%d"), "prediction": prob}
    )
...
def create_environment():
    ...
    try:
        param = str(lon + "," + lat)
        generate_data_and_heightmap(param, date)
    except Exception as e:
        return jsonify({"error": "Heightmap generation failed", "details": str(e)}), 500
    ...
    if SPAWN_SIMULATOR:
        subprocess.Popen([sys.executable, "-m", SIMULATOR_MODULE], env=env)
    return jsonify({"lat": lat, "lon": lon, "date": date})
