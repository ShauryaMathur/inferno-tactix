...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compare paid OpenRouter models on FireCastBot RAG prompts using a Gemini judge."
    )
    parser.add_argument(
        "--prompts",
    ...
    parser.add_argument(
        "--models",
        default="apps/api/evals/models.openrouter.paid.json",
        help="Path to model records JSON/JSONL. Each item needs provider and model.",
    )
    parser.add_argument(
        "--incident-report",
        default="apps/firecastbot/incident_reports/incident_report_boulder.pdf",
        help="Path to an incident report PDF to include runtime incident retrieval.",
    )
    parser.add_argument(
        "--doctrine-manifest",
