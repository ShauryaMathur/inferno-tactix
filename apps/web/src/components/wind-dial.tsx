...
import { CircularInput, CircularTrack } from 'react-circular-input';
...
export const WindDial = ({ windDirection, onChange, onChangeEnd }: IProps) => {
  ...
  const circularInputValToAngle = (circularInputVal: number) =>
    (circularInputVal * 360 + 180) % 360;
  const circularInputValue = () => (windDirection - 180) / 360;
  const onChangeConvertedValue = (circularInputVal: number) =>
    onChange?.(circularInputValToAngle(circularInputVal));
  const onChangeEndConvertedValue = (circularInputVal: number) =>
    onChangeEnd?.(circularInputValToAngle(circularInputVal));
