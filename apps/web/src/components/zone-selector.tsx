...
const getBackgroundImage = (zoneCount: number, terrainType: number, currentZone: number) => {
  ...
  const panelPosition =
    zoneCount === 2 ? twoZonePosition[currentZone] : threeZonePosition[currentZone];
      ...
      zoneUI.push(
        <div className={`${css.zone} ${cssClasses[i]} ${zoneStyle}`} key={i}>
          <label className={css.terrainPreview}>
            ...
            <div
              className={`${css.terrainImage} ${getColorFilter(z.droughtLevel)}`}
              style={{ backgroundImage: `url(${zoneTerrainImagePath})` }}
            >
              <div
                className={`${css.riverOverlay}`}
                style={{ backgroundImage: `url(${zoneRiverImagePath})` }}
              />
              {!readonly && (
