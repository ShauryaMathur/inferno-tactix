...
export const FireLineMarkersContainer: React.FC<IProps> = observer(function WrappedComponent({
  dragPlane,
}) {
  ...
  return (
    <>
      {simulation.fireLineMarkers.map((fl, idx) => {
        ...
        const onDragEnd = () => {
          const firelineStartPoint =
            idx % 2 === 0 ? simulation.fireLineMarkers[idx] : simulation.fireLineMarkers[idx - 1];
          const firelineEndPoint =
            idx % 2 === 0 ? simulation.fireLineMarkers[idx + 1] : simulation.fireLineMarkers[idx];
          log('FireLineUpdated', {
