...
export const TopBar: React.FC<IProps> = ({ projectName, aboutContent, shareContent }: IProps) => {
  ...
  return (
    <div className={css.topBar}>
      <span className={css.textButton} data-testid="reload" onClick={handleReload}>
        <RefreshIcon />
      </span>
      <span>
        <span data-testid="share" className={css.textButton} onClick={handleShareOpen}>
          Share
        </span>
        <span data-testid="about" className={css.textButton} onClick={handleAboutOpen}>
          About
        </span>
      </span>
      <Dialog onClose={handleAboutClose} open={aboutOpen} title={`About: ${projectName}`}>
        {aboutContent}
      </Dialog>
      <Dialog onClose={handleShareClose} open={shareOpen} title={`Share: ${projectName}`}>
        {shareContent}
      </Dialog>
    </div>
  );
