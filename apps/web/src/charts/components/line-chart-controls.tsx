...
export class LineChartControls extends BaseComponent<IChartControlProps, IChartControlState> {
  public static getDerivedStateFromProps: any = (
    nextProps: IChartControlProps,
    prevState: IChartControlState
  ) => {
  ...
  public render() {
    ...
    const timelineVisible =
      chartData.maxPoints && chartData.maxPoints > 0 && chartData.pointCount > chartData.maxPoints;
    ...
    return (
      <div className={css.lineChartControls} id="line-chart-controls">
        ...
        <div className={css.toggleDataSubset} onClick={toggleShowAllOrRecent}>
          {toggleButtonText()}
        </div>
      </div>
    );
