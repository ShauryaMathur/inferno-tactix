...
export const populateGrid = (
  ...
  const numGridCellsPerImageRowPixel = interpolate
    ? (imageHeight - 1) / (height - 1)
    : imageHeight / height;
  const numGridCellsPerImageColPixel = interpolate
    ? (imageWidth - 1) / (width - 1)
    : imageWidth / width;
  ...
  for (let r = 0; r < height; r++) {
    ...
    for (let c = 0; c < width; c++) {
      ...
      if (interpolate) {
        ...
        const bottomRight =
          imageColIndex + 1 < imageWidth ? image[imageRowIndex][imageColIndex + 1] : bottomLeft;
        const topLeft =
          imageRowIndex - 1 >= 0 ? image[imageRowIndex - 1][imageColIndex] : bottomLeft;
        const topRight =
          imageRowIndex - 1 >= 0
            ? imageColIndex + 1 < imageWidth
              ? image[imageRowIndex - 1][imageColIndex + 1]
              : topLeft
            : bottomRight;
        value =
...
export const getInputData = (
  ...
  return new Promise((resolve) => {
    if (input === undefined) {
      ...
      // input is a string, URL to an image
      getImageData(input as string, mapColor, (imageData: number[][]) => {
