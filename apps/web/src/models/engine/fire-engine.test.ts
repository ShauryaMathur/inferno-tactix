...
describe('getGridCellNeighbors', () => {
  it('returns array of neighbours without cells that are nonburnable, or lay behind them', () => {
    ...
    expect(getGridCellNeighbors(cells, 5, 4, 4, 1.5, BurnIndex.Low).sort()).toEqual([
      0, 1, 2, 4, 6, 8,
    ]);
    expect(getGridCellNeighbors(cells, 5, 4, 4, 2.5, BurnIndex.Low).sort()).toEqual([
      0, 1, 12, 2, 3, 4, 6, 7, 8,
    ]);
...
describe('FireEngine', () => {
  ...
  describe('fire survivors', () => {
    ...
    it('should not mark any forest with suppression cells as fire survivors', () => {
      expect(testVegetationAndGetNumberOfFireSurvivors(Vegetation.ForestWithSuppression)).toEqual(
        0
      );
    });
  });
