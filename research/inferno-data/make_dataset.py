...
def load_cache() -> Dict[Tuple[str, int], Any]:
    ...
    try:
        ...
        logging.info("Loaded cache (%d entries) from %s", len(cache), config.CACHE_PICKLE_PATH)
        return cache
    except Exception:
...
def save_cache():
    ...
    try:
        ...
        logging.info("Saved cache (%d entries) to %s", len(_DATA_CACHE), config.CACHE_PICKLE_PATH)
    except Exception:
...
def _fetch_worker(
    feat: str, yr: int
) -> Tuple[str, int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    try:
        ...
        dates = np.array(
            [
                np.datetime64(datetime(d.year, d.month, d.day), "D")
                for d in num2date(raw, units=units)
            ],
            dtype="datetime64[D]",
        )
...
# WINDOW EXTRACTOR ─────────────────────────────────────────────────
def extract_window(lat: float, lon: float, when: datetime, feat: str) -> pd.Series:
...
def main():
    ...
    if to_fetch:
        ...
        if not os.path.exists(config.CACHE_PICKLE_PATH) and config.SAVE_CACHE:
            save_cache()
    else:
