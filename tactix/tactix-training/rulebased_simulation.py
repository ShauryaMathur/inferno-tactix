...
print("🚁 Starting RULE-BASED SIMULATION")
...
class FireEnvSync(gym.Env):
    ...
    def _default_state(self):
        return {
            'helicopter_coord': np.array([30, 70], dtype=np.int32),
    ...
    def step(self, action,heli_x,heli_y):
        self.step_count += 1
...
def generate_circle_path(origin, radius):
    points = []
    for angle in range(0, 360, 10):
        rad = np.deg2rad(angle)
        x = int(origin[0] + radius * np.cos(rad))
        y = int(origin[1] + radius * np.sin(rad))
        points.append((x, y))
        points.append((-69,-69))
        points.append((-69,-69))
    return points[:-2]
    ...
    try:
        ...
        for episode in range(NUM_EPISODES):
            ...
            while not done and step < MAX_TIMESTEPS:
                ...
                obs, reward, done, truncated, info = env.step(action,x,y)
